  ...
  window.addEventListener('load', function () {
    ...
    Shape.prototype.draw = function () {
      ...
      for (var j = 0; j < 4; j++) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          Math.cos(rad * 0) * this.r + this.x,
          Math.sin(rad * 0) * this.r + this.y + this.r
        );
        for (var i = 1; i < 4; i++) {
          if (i !== 2) {
            ctx.lineTo(
              Math.cos(rad * i) * this.r + this.x,
              Math.sin(rad * i) * this.r + this.y + this.r
            );
          }
        }
        ctx.closePath();
    ...
    canvas.addEventListener(
      ...
      function (e) {
        ...
        for (var i = 0; i < num; i++) {
          var b = new Bubble(
            ctx,
            rand(mouseX - 100, mouseX + 100),
            rand(mouseY - 100, mouseY + 100)
          );
          bubbles.push(b);
        }
      },
